...
{
    public class ParserBestPriceTour
    {
        /// <summary>
        ...
        public async Task<List<ContentDTO>> GetListContents(string html)
        {
            ...
            try
            {
                ...
                string zone = Utilities.Xpath_GetHTML(html, "//ul[contains(@class,'deal-item')]", false);

                // lấy danh sách bài viết trong vùng mới giới hạn zone ở trên
                List<string> listHtmlContent = Utilities.Xpath_GetListHTML(zone, "//li[contains(@class,'tour-ids')]");
                ...
                if (listHtmlContent != null && listHtmlContent.Count > 0)
                {
                    ...
                    {
                        string subject = Utilities.Xpath_GetHTML(contentHtml, "//h3", true); // truyền vào 1 đoạn xpath, xử lý xong trả ra đoạn text thỏa mã đoạn xpath đó, giá trị true để xóa đi các thẻ html giữ lại đoạn text

                        string url = Utilities.Xpath_GetHTML(contentHtml, "//li[contains(@class,'tour-ids')]", false);
                        url = Utilities.Regex_GetTextFromHtml(url, "(data-go-url[\\s]{0,}=[\\s]{0,}('|\")).*?('|\")", "", "(data-go-url[\\s]{0,}=[\\s]{0,}('|\"))|('|\")", true);
                        if (string.IsNullOrEmpty(url))
                        {
                            url = Utilities.Regex_GetTextFromHtml(url, "data-go-url=(.*?)(.html|.htm)", "", "data-go-url=", true);
                        }
            ...
            try
            {
                ...
                for (int i = 0; i < dataUrl.Count; i++)
                {
                    ...
                    if (status == 0)// check xem đã bóc hay chưa?
                    {
                        ...
                        string subject = Utilities.Xpath_GetHTML(html, "//div[contains(@class,'container')]//h1", true); // truyền vào 1 đoạn xpath, xử lý xong trả ra đoạn text thỏa mã đoạn xpath đó, giá trị true để xóa đi các thẻ html giữ lại đoạn text             

                        string content = Utilities.Xpath_GetHTML(html, "//div[contains(@data-page,'overview')]", true);

                        string totalPoint = Utilities.Xpath_GetHTML(html, "//div[contains(@class, 'col-xs-12')]//div[contains(@class, 'review-number')]/span", true);

                        string contentHtml = Utilities.Xpath_GetHTML(html, "//div[contains(@class,'fotorama__stage__frame')]", false); // nội dung không xóa thẻ Html

                        string imageThumb = "";
                        // trong nội dung bài viết có nhiều hình, cần tìm hình đầu tiên ra làm hình đại diện
                        List<string> lisImg = Utilities.Xpath_GetListHTML(contentHtml, "/img");
                        if (lisImg != null && lisImg.Count > 0)
                        {
                            foreach (string imgHtml in lisImg)
                            {
                                string imageThumbHtml = Utilities.Regex_GetTextFromHtml(imgHtml, "src[\\s]{0,}=[\\s]{0,}\".*?\"", "", "src[\\s]{0,}=[\\s]{0,}\"|\"", true);

                                // sau khi lấy url chỉ là tương đối, để rõ ràng cần nối thêm cái đầu https://www.bestprice.vn/ vào nữa
                                imageThumbHtml = Utilities.FixPathImage(imageThumbHtml, "https://www.bestprice.vn/");
                        ...
                        objReturn.ReferUrl = url;//url bài viết
        ...
        public ParserBestPriceTour()
        {
        }
        public ParserBestPriceTour(ChromiumWebBrowser browser)
        {
            _browser = browser;
        }
                        ...
                        while (true)
                        {
                            ...
                            if (divComment != null)
                            {
                                foreach (HtmlNode item in divComment)
                                {
                                    ...
                                    comment.ContentsComment = Utilities.RemoveSpecialCharacter(item.SelectSingleNode(".//div[contains(@class,'cus-rv-content')]/span")?.InnerText);
                            ...
                            string checkJs = await Utilities.EvaluateJavaScriptSync(_jsClickShowMoreReview, _browser).ConfigureAwait(false);
                            if (checkJs == null)
                            {
                                break;
                            }
